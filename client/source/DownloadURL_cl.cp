...
//	soundsversion -= 0x200;
	...
	// nothing new?
	bool bNeedImages = ( imagesversion < BigToNativeEndian( msg->logImagesVersion ) );
	bool bNeedSounds = ( soundsversion < BigToNativeEndian( msg->logSoundsVersion ) );
	...
	if ( noErr == result )
		...
		if ( bNeedImages )
			...
			//
			// (We keep "perfect" updates for the last three versions on the website,
			// e.g. for v100 we'd have individual delta files for 97->100, 94->100,
			// and 91->100, in addition to the full complete v100 image data file.
			// So even if you haven't played for a couple of months, we can still
			// fix you up with a fairly small, incremental patcher.
			//
			//	For a while we also maintained a v64->vCurrent incremental file, because
			// v64 is what was pressed onto the physical Clan Lord CDs, and so we could
			// assume that everyone had at least that. But due to image churn, there's
			// no longer any space/time savings there, compared to fetching the
			// complete images file from scratch.)
			//
			neededversion = BigToNativeEndian( msg->logImagesVersion ) >> 8;
			snprintf( filename, sizeof filename,
				"%s.%dto%d.gz", kClientImagesFName, (imagesversion >> 8), neededversion );
		...
		// there were no file-size savings compared to the whole file updater
			...
			&&   imagesversion > (uint) kVersionCD )
				{
				snprintf( filename, sizeof filename,
					"%s.%dto%d.gz", kClientImagesFName, (kVersionCD >> 8), neededversion );
				result = AutoUpdateDataFile( msg, dir, filename );
				if ( noErr == result )
					result = UpdateImagesFile( neededversion );
				}
		...
		if ( noErr == result  &&  bNeedSounds )
			...
			neededversion = BigToNativeEndian( msg->logSoundsVersion ) >> 8;
			snprintf( filename, sizeof filename,
				"%s.%dto%d.gz", kClientSoundsFName, (soundsversion >> 8), neededversion );
			result = AutoUpdateDataFile( msg, dir, filename );
			if ( noErr == result )
			...
			&&   soundsversion > (uint) kVersionCD )
				{
				snprintf( filename, sizeof filename,
					"%s.%dto%d.gz", kClientSoundsFName, (kVersionCD >> 8), neededversion );
				result = AutoUpdateDataFile( msg, dir, filename );
				if ( noErr == result )
					result = UpdateSoundsFile( neededversion );
				}
	...
	if ( noErr == result )
		{
		if ( ( bNeedImages && uint(gImagesVersion) < BigToNativeEndian( msg->logImagesVersion ) )
		||   ( bNeedSounds && uint(gSoundsVersion) < BigToNativeEndian( msg->logSoundsVersion ) ) )
			{
			// "The downloaded update files seem to be corrupt. Please contact joe@deltatao.com."
...
AutoUpdateDataFile( const LogOn * msg, const char * dir, const char * findme )
{
	...
	for(;;)
		...
		if ( 0 == memcmp( filename, findme, len ) )
			...
			// download it
			char url[ 512 ];
			snprintf( url, sizeof url, "%s%s/%s", baseURL, kDataSubDirectory, filename );
			DTSFileSpec spec;
...
*/
const char *
ExtractFileName( const char * start, const char * filename, char * buff, size_t size )
{
...
MoveFileSafer( DTSFileSpec * source, DTSFileSpec * dest )
{
	...
	for (;;)
		...
		// else there was a file system error, and we might as well stop everything now.
		if ( result != noErr )
			break;
		}
...
AbdicateToNewClient( CFURLRef u )
{
	...
	if ( CFStringRef leaf = CFURLCopyLastPathComponent( u ) )
		{
//		"A new version of the " TXTCL_APP_NAME " client with the name " LDQUO "%@" RDQUO
//		" has been downloaded, and your current client has been moved into the trash. "
//		"This old client will now quit, and the new one will start up."
...
//	if ( noErr == result )
	...
	if ( noErr == result )
		...
		result = AESend( &quitMe, &reply, kAENoReply, kAENormalPriority, kNoTimeOut,
					nullptr, nullptr );
		__Check_noErr( result );
		AEDisposeDesc( &reply );
		}
...
// are we using fork()/exec(), or the newer preferred alternative, namely posix_spawn()?
...
# endif
		...
		if ( -1 == result )
			...
			// the only safe thing left is to _exit() -- even plain old exit() is too much.
			_exit( EXIT_FAILURE );
			}
		}
...
**	(in particular, the OS doesn't seem to appreciate it if the current, live client
**	attempts to move _itself_ into the trash).
*/
void
LaunchNewClient()
{
...
//	if ( noErr == err )
	...
	if ( noErr == err )
		{
		if ( CFURLRef u = CFBundleCopyAuxiliaryExecutableURL( bndl, CFSTR(HELPER_TOOL_NAME) ) )
			{
			if ( not CFURLGetFileSystemRepresentation( u, true,
						reinterpret_cast<uchar *>( helper_path ), sizeof helper_path ) )
				{
				err = coreFoundationUnknownErr;
				}
			CFRelease( u );
...
**	That will download the url into the CFData. You're responsible for disposing of
**	the CFData after you're done with it, obviously.
**	This mode is typically used when scanning directory listings.
*/
DTSError
InternalDownloadURL( const char * url, DTSFileSpec * destSpec, CFMutableDataRef destData )
{
#if 0 && defined( DEBUG_VERSION )
	...
	if ( destSpec )
		...
		// no cross-device copies, but could break if the client is on a read-only volume
		...
		if ( noErr == result )
			{
			// TO DO: double-check that we're not outsmarting ourselves with the following.
	...
	if ( noErr == result )
		...
		if ( CFURLRef urlRef = CFURLCreateWithBytes( kCFAllocatorDefault,
									reinterpret_cast<const uchar *>( url ), strlen( url ),
...
ZLibExpand( DTSFileSpec * spec, bool inDeleteOriginal /* =false */ )
{
	...
	if ( noErr == result )
		{
		// the type code of 0 is meant to make the finder treat the file as "temporary"
		// see TN 1142, "Mac OS 8.5"
		result = FSDecodeMacBinary( &binRef, gDTSApp->GetSignature(), 0, smSystemScript );
...
**	ExpandFile()
**
**	unstuff a file using zlib + optional macbinary
...
CreateURLForUnTarballedClient( CFURLRef compressed, CFURLRef& outURL )
{
	...
	if ( noErr == err )
		...
		*/
		...
		int result = asprintf( &cmd, "cd \"%s\"; /usr/bin/tar xzf \"%s\"", tmpPath, base );
		if ( -1 == result || not cmd )
			err = memFullErr;
		__Check_noErr( err );
		}
...
# define NEW_CLIENT_NAME	"ClanLord+.app"
		...
		if ( CFURLRef u = CFURLCreateCopyDeletingLastPathComponent( kCFAllocatorDefault,
								compressed ) )
			{
			// ".../Downloads/ClanLord.app/"
			outURL = CFURLCreateCopyAppendingPathComponent( kCFAllocatorDefault,
...
// Much of this was lifted from Apple sample code here:
//	<http://developer.apple.com/samplecode/CFNetworkHTTPDownload/CFNetworkHTTPDownload.html>
// (version of 8/10/2005)

// how long to wait for URL data before giving up on the connection (seconds)
/*
	originally this was just 10 seconds. But it seems that on very slow connections
	(eg. noisy dialup), and if the OS internally uses very large data buffers, at it might,
	then there can be an arbitrarily long time between each HasBytesAvailable notice.
	So I've decided to use a ridiculously long timeout -- effectively, none at all --
	on the theory that
		a) if the connection _truly_ dies, we'll get an Error or End event, and can close
			things down cleanly;
		b) if the user, watching the progress bar, decides things are wedged, he can always
			cancel the download manually; and therefore
		c) if neither of the above happens, we should let the transfer continue, no matter
			how slow.
*/
...
static EventLoopTimerRef	sNetworkTimer;		// to terminate a download that fails in midstream
static volatile bool		bCFDownloadInProgress;	// true iff we're doing a download
...
struct DLContext
{
	...
	CFIndex		expectedLen;	// from remote HTTP server. 0: unknown, -1: not available
};
...
**	GetLenFromHeader()
**
**	after we get our first trickle of data from the CFReadStream, we can interrogate
**	it to see if there's a "Content-Length" header in the HTTP response.
**	If so, that'll tell us the final expected length of the stream.
...
GetLenFromHeader( CFReadStreamRef stream, volatile DLContext * c )
{
	...
	if ( not got )
		...
		// Record that fact in the context, to prevent further length-measurement attempts.
		c->expectedLen = -1;
		}
}
...
**
**	called by the stream (or rather by the runloop on which the stream is scheduled)
**	whenever any notable stream event takes place.
**	If there is some fresh data, we'll save it and update the progress values.
**	If anything goes wrong, or we arrive at the end of the transfer, close up shop.
**	For now, just ignore everything else.
*/
static void
ReadStreamClientCallback( CFReadStreamRef stream, CFStreamEventType evtType, void * ud )
{
	try
		{
		...
		switch ( evtType )
			...
			case kCFStreamEventHasBytesAvailable:
				...
				// First we'll try to read whatever (internal) data the stream object 
				// may already have cached for us. If that fails, we'll do an explicit read
				// into our own buffer, at the cost of some duplicative memcpy()'s.
				...
				const uchar * data = CFReadStreamGetBuffer( stream, kAsMuchAsYouCan, &bytesRead );
				if ( not data )
					{
				...
				if ( bytesRead > 0 )
					...
					if ( sNetworkTimer )
						{
						// FIXME/TODO?
						// maybe recalibrate this timeout based on current overall data rate?
						// We'd have to know the size of CFNetwork's internal buffers, though...
					...
					if ( c->refnum > 0 )
						{
						err = FSWriteFork( c->refnum, fsAtMark, 0, bytesRead, data, nullptr );
						__Check_noErr( err );
						}
					else
					if ( c->h )
...
//					c->lastErr = streamErr.error;
					...
					{
					// error occurred, but no CFErrorRef? You got some 'splainin' to do!
					c->lastErr = coreFoundationUnknownErr;
					}
				
				c->cumulativeErr = kNetworkDisconnect;		// again, maybe not the best
				TerminateDownload( stream, c );
				}
				break;
...
TerminateDownload( CFReadStreamRef stream, volatile void * ctxt )
{
	...
	__Verify_noErr( SetEventLoopTimerNextFireTime( sNetworkTimer, kEventDurationForever ) );
...
**	if 'destDataRef' is non-NULL, we'll download into it, and 'destRef' & 'fname' are ignored.
				 ...
				 CFMutableDataRef destDataRef )
{
	...
	if ( not destDataRef )
		...
		result = FSCreateFileAndOpenForkUnicode( &destRef, fname.length, fname.unicode,
	...
	// stream context record (we'll only be using its 'info' (i.e. "refcon") field)
	CFStreamClientContext ctxt;
	memset( &ctxt, 0, sizeof ctxt );
	
	// These are the network events we're interested in.
	const CFOptionFlags kOurNetworkEvents = /* kCFStreamEventOpenCompleted | */
											kCFStreamEventHasBytesAvailable |
											kCFStreamEventEndEncountered |
											kCFStreamEventErrorOccurred;
	
	// user-data for the timer callback: what it needs to be able to kill off the D/L.
	TimerCtxt timerCtxt;
	
	// the CFRunloop on which our stream will be scheduled
	CFRunLoopRef curLoop = (CFRunLoopRef) GetCFRunLoopFromEventLoop( GetCurrentEventLoop() );
	
	// create an HTTP "GET" request for the desired URL
	CFReadStreamRef readStreamRef = nullptr;
	CFHTTPMessageRef messageRef = CFHTTPMessageCreateRequest( kCFAllocatorDefault,
	...
	CFReadStreamScheduleWithRunLoop( readStreamRef, curLoop, kCFRunLoopCommonModes );
...
//			ShowMessage( "Bytes read: %ld of %ld", info.bytesRead, info.expectedLen );
...
#if 0 && defined( DEBUG_VERSION )
	ShowMessage( "D/L done; total bytes: %ld, error: %d", info.bytesRead, int(result) );
#endif
...
Bail:
	...
	if ( readStreamRef )
		...
		CFReadStreamUnscheduleFromRunLoop( readStreamRef, curLoop, kCFRunLoopCommonModes );
