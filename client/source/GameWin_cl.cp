...
public:
...
#ifdef	TRANSLUCENT_HEALTH_BARS
	// number of ticks the full bars are still displayed before disappearing (gossip mode)
	...
	void	ShowDrawTime();
			...
			// but CL players type all kinds of weird stuff
			enum { kFontArraySize = 256 };	// actually, lastID-firstID+1, 255-0+1
...
protected:
	GLuint	Make1FontList( AGLContext ctx, FMFontFamily fontID, int fsize, Style face );
...
public:
	...
	bool			winInRedraw;	// true iff redrawing data from new packet; false for updtEvts
...
enum
{
	...
	kLanguageVerbTypeRange = 6,
		// FillUnknownLanguage() uses sizeof "gLanguageTableShort / sizeof gLanguageTableShort[0]"
...
/*
	to-do: improve the thieves' language transcriptions.
...
**
**	with one key frame and one delta frame you're doubling the latency when a packet is lost.
...
**
**	the other major problem with delta frame concept is that it requires the server
**	do do more work. ie compute the deltas.
...
**	if kMaxStoredFrame is changed to anything other than 1 then CCLFrame::ReadKeyFromSpool
**	will need to be modified (somehow!).
**	probably by copying the static images from the previous frame to the current one.
**
**	-timmer
*/
...
const uchar				gBubbleMap[] =
{
	...
	3,		// kBubblePonder - the ponder bubble uses the same graphic as the thought bubble
...
using std::sin;
	...
	// doing it like this means it is done at compile time, not every time the app initializes
...
CreateCLWindow( const DTSRect * pos )
{
	...
	// create window of type document
		// normally we do want get-front-click, but if you use click-toggles mode, then
		// you're too often apt to find yourself running aimlessly.
...
#define	TXTCL_GAME_WINDOW_NAME	TXTCL_APP_NAME
	...
	// for some unfathomable reason. Then he unfriends all the people who really ARE his friends.
	// Or something like that.  Anyway, this flag lets him do that without too much trauma.
...
#ifdef OGL_SHOW_DRAWTIME
	// look for the special development preference, so even GMs don't see the drawtimes
	// unless they specifically ask for it, via:
...
DynamicInitCLWindow()
{
	...
	win->winOffView.offBufferImage.Init( nullptr, gLayout.layoWinWidth, gLayout.layoWinHeight,
			kCLDepth );
	CheckError();
	...
	win->winOffView.offStatusBar.Init( nullptr,
				gLayout.layoHPBarBox.rectRight - gLayout.layoHPBarBox.rectLeft,
				gLayout.layoHPBarBox.rectBottom - gLayout.layoHPBarBox.rectTop + 2, kCLDepth );
	CheckError();
	...
	win->winTextField.Move( gLayout.layoTextBox.rectLeft, gLayout.layoTextBox.rectTop );
	win->winTextField.Size( gLayout.layoTextBox.rectRight - gLayout.layoTextBox.rectLeft,
							gLayout.layoTextBox.rectBottom - gLayout.layoTextBox.rectTop );
...
RedrawCLWindow()
{
	...
	static int oldFastLimit, oldSlowLimit;		// watch that our globs didn't change (hack!)
	...
	// if using opengl, we need to make sure nothing needs to be changed for this frame
#ifdef USE_OPENGL
	setOGLState();
#endif	// USE_OPENGL
...
//	gFastBlendMode = ( (totalTime / sampleCount) >= 245 );	// downshift if avg time is long...
//	gFastBlendMode = ( time >= 245 );						// downshift if this frame was long
}
...
CLWindow::DoDraw()
{
	...
	// (i.e. if this was called from an updateEvt or kEventWindowDrawContent) then
	// draw everything but the text field and the input field (which will draw themselves)
	DTSRect box;
	if ( not winInRedraw )
		{
		DTSRegion mask;
		...
		if ( gUsingOpenGL )
			{
			aglSwapBuffers( ctx );
			ShowOpenGLErrors();	// on general principle, must call this at least once per frame
...
# if defined(USE_OPENGL) && defined(DEBUG_VERSION) && defined(OGL_SHOW_IMAGE_ID)
			ShowMessage( "swap buffer" );	// for finding problem images when using light casters
...
// when no players are selected
	...
	if ( not newPlyrName || 0 == newPlyrName[0] )
		...
		switch ( friends )
			...
			case kFriendLabel4:
			case kFriendLabel5:
				menu->CheckRange( mNoLabel, mPurpleLabel, mRedLabel + friends - kFriendLabel1 );
				break;
...
MovePlayerToward( int quadrant, int moveSpeed, bool /* stopIfBalance */ )
{
	...
	int speed = ( gLayout.layoFieldBox.rectRight - gLayout.layoFieldBox.rectLeft ) / 2;
...
CLWindow::Click( const DTSPoint& loc, ulong time, uint modifiers )
{
	...
	if ( loc.InRect( &gLayout.layoFieldBox ) )
		...
		if ( modifiers & (kKeyModMenu | kKeyModOption | kKeyModControl) )
			...
			switch ( modifiers & (kKeyModMenu | kKeyModOption | kKeyModControl | kKeyModShift) )
				{
				...
				case kKeyModControl :
					...
					// shift-control-click: advance label color in the other direction
...
//	if ( gClickState ) - Jeff weird mouse control when clicking after a movement macro
...
CLWindow::KeyStroke( int ch, uint modifiers )
{
	...
	if ( CCLMovie::IsReading() && (modifiers & kKeyModMenu) )
		...
		switch ( ch )
			...
			case kRightArrowKey:
				CCLMovie::SetReadSpeed( CCLMovie::GetReadSpeed() + 1 );
	...
	if ( loc.InRect( &gLayout.layoFieldBox ) )
		...
		if ( true )
			...
			if ( not gHoveringOnPlayer || gHoveringOnPlayer != desc )
				...
				{
				int hoverDelay = (desc == gThisPlayer) ? kNameDelay_HoverSelf : kNameDelay_Hover;
...
{
	UInt32 macMods = 0;
	(void) evt.GetParameter( kEventParamKeyModifiers, typeUInt32, sizeof macMods, &macMods );
...
CLWindow::DoMouseDownEvent( const CarbonEvent& evt )
{
	...
	//if ( noErr == err )
		...
		err = evt.GetParameter( kEventParamMouseButton, typeMouseButton, sizeof button, &button );
		__Check_noErr( err );
		}
	...
	if ( noErr == err )
		{
		err = evt.GetParameter( kEventParamMouseChord, typeUInt32, sizeof chord, &chord );
		
		// we'll probably get an error on OS 9 here; if so, don't make a fuss
		if ( eventParameterNotFoundErr == err )
			{
			chord = 1U << (button - 1);		// make it look like a single-button event
			err = noErr;
			}
		__Check_noErr( err );
		}
...
//		loc.Offset( -winPos.ptH, -winPos.ptV );
		...
		// The event system on X magically transmutes right-clicks (ie button #2)
		// into control-clicks (of button #1), but only when the event is fetched
		// via WaitNextEvent(). However, if the event is received via a carbon-event
		// mousedown handler (such as this very stretch of code right here:
		// what a coincidence!), we get told the real facts, not that little white lie.
		//
		// What that means is, if we were to return noErr here after receiving a #2 click,
		// the control-conversion wouldn't happen, since the click wouldn't ever
		// propagate through to WaitNextEvent().
		//
		// The problem is, players have lots of existing macros mapped to ctrl-click
		// which they rely on being able to trigger with their #2 button, so we
		// have to make sure that we don't break them.
		
		// So... if the button number was 3 or higher (we already rejected #1, above),
		// or if a macro has _specifically_ handled this #2-click,
		// we'll return noErr, so that the event is considered fully handled and won't
		...
		// player has a macro specifically bound to click#2, AND if that macro has its
		// $no_override attribute set, AND they also have a macro bound to ctrl-click,
		// THEN DoMacroClick() above will run the first macro and return 'false';
		// we'll correctly return the not-handled code; the click will get turned into
		// a control-click in WNE; and their other macro will _also_ run. Two macros
...
CLWindow::DoWheelEvent( const CarbonEvent& evt )
{
	...
	if ( noErr == err )
		{
		err = evt.GetParameter( kEventParamMouseWheelDelta, typeSInt32, sizeof delta, &delta );
		__Check_noErr( err );
		}
...
# endif	// OGL_USE_UPDATE_OVERRIDE
		glClear( GL_COLOR_BUFFER_BIT | GL_STENCIL_BUFFER_BIT );	// harmless if no stencil buffer
	...
	&&   gOGLStencilBufferAvailable )
		{
			...
			// we're going to reduce the alpha test in stages, drawing the darkest areas first
...
# ifdef OGL_QUICKDRAW_LIGHT_EFFECTS
			...
			{
// the CopyDeepMask path is twice as slow, but looks much better.  much, much better.
// the qdApplyLightmap path looks awful, but is twice as fast.
// i'd be inclined to stay with the CopyDeepMask, but the whole point of this exercise
// is to support slow computers, and we already have the gl client for folks at the other
// of the spectrum who want "pretty".
//
// update: the client already has support for selecting between fast/ugly and slow/quality
// blitters (gFastBlendMode), so we can just use that.

// next up: figure out some way to get rid of gAllBlackGWorld (because it contributes nothing
// to the end result; it's only there to ensure the first half of the blend equation does nothing)
//
// next next up: create a secondary 32 bit color buffer to use in place of the 8 bit buffer,
// which will eliminate the 32->8 bit conversion, as well as eliminating the nasty banding
// we are getting.  (if the user has their monitor set to 8 or 16 bit color, the OS/video system
// will do a downsample for us anyway.)  the only downside will be that some elements
// (speech balloons, etc.) are drawn after the night shading, and will need 32-bit versions
// to support this new scheme.
			...
			if ( gFastBlendMode )
				...
				::CopyDeepMask(
					...
					DTS2Mac(&gLayout.layoFieldBox),
					DTS2Mac(&gLayout.layoFieldBox),
					srcCopy + ditherCopy	// equivalent to ditherCopy alone, since srcCopy == 0
//					srcCopy					// this makes it look just like qdApplyLightmap()
					nullptr
				);
...
# endif  // OGL_QUICKDRAW_LIGHT_EFFECTS
	...
	// one or both of DrawHandObjects() and DrawBar() may be done in a future ogl version,
	// but not now
	...
	BlitTest( this, &offSmallBubbleImage->icImage.cliImage, &offSmallBubbleImage->icBox, &dst );
	}
#endif  // 0
...
//			( nightTargetLevel == 0 ) && not gNumDarkCasters )
		...
		if ( nightTargetLevel )	// only if there is any night to draw...
			...
			GLfloat nightTargetRedshift = gNightInfo.GetMorningEveningRedshift();
				// if the night level in the displaylist isn't the night level we want to show
				...
				( nightListRedshift != nightTargetRedshift ))
			{
				...
				GLfloat rimColor = 1.0f - nightListLevelf;
					// note that the maximum center color is reduced to 0.5 (down from 1.0)
					// because the remainder is expected to come from exile lighting
					// in this special case though, there are neither light nor dark casters
				GLfloat centerColor;
...
// we start to implement exile-with-no-lights
		...
		GLfloat redshiftClearColorf = clearColorf * gNightInfo.GetMorningEveningRedshift();
...
# else
		CreateShadow( this, &cache->icImage.cliImage, &cache->icBox, &dst, &gFieldCenter,
			kPictDefShadowNormal /* cache->icImage.cliPictDef.pdFlags */ );
# endif  // USE_OPENGL
		}
...
#endif	// USE_OPENGL
					Draw( &cache->icImage.cliImage, kImageCopyFast, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					BlitTransparent( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					FastBlitBlend25( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					FastBlitBlend50( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					FastBlitBlend75( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					FastBlitBlend25Transparent( this,
...
#endif	// USE_OPENGL
					FastBlitBlend50Transparent( this,
						&cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					QualityBlitBlend25( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					QualityBlitBlend50( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
#endif	// USE_OPENGL
					QualityBlitBlend75( this, &cache->icImage.cliImage, &cache->icBox, &dst );
				break;
...
# endif
	...
	&&   srcBox.rectBottom <= icBounds.rectBottom )
		...
		if ( ic->icImage.cliPictDef.pdFlags & kPictDefFlagUprightShadow )
			...
			if ( whichPose >= 0 )	// temporary hack to not display shadows for /pose dead or lie
				{
				DTSRect shadowRect;
				...
				if ( whichPose != state )
					{
					shadowRect.rectTop  = ic->icBox.rectTop + mobileSize * (whichPose >> 4);
...
#if defined(USE_OPENGL) && defined(DEBUG_VERSION) && defined(OGL_SHOW_IMAGE_ID)
	ShowMessage( "upright shadow mobile ID: %d, state: %d", (int) desc->descID, whichPose );
#endif	// USE_OPENGL && DEBUG_VERSION && OGL_SHOW_IMAGE_ID
				}
			else
				{
...
DrawGLHilite( const DTSRect& box, int& openGLEnableEffectsState )
{
	...
	glPushMatrix();
		...
		if ( openGLEnableEffectsState != int(gOpenGLEnableEffects) )
			...
			// this is a lot easier to understand if you plot a few points on some graph paper
			for ( int i = 0; i <= 11; ++i )
				{
				int index = i + 5;
			...
			glNewList( gOGLSelectionList, GL_COMPILE_AND_EXECUTE );
				...
				glBegin( GL_TRIANGLE_STRIP );
					...
					for ( int i = 0; i <= 16; ++i )
						{
						if ( openGLEnableEffectsState )
							glColor4us( 0x0, 0x0, 0xFFFF, 0xFFFF * (1.0f - (i + 1) / 18.0f) );
...
CLOffView::Draw1Name( const DSMobile * mobile )
{
	...
	// regular clients have no access to that setting, so they'll never have it on.
	// So players will never see monster names unless a script specifically wants them to.
	// GMs can change the pref if desired (via the debug client); the value will persist
	// even unto future clients, both regular and debug.
	// Therefore GMs will either see all names, or no names.
	// Come to think of it, the really flexible way to handle all this is at the server,
	// via an "/options" flag that's consulted when assembling the drawstates.
	// Oh well.
		...
		if ( not desc->descSeenFrame )		// not seen him, or long ago
			...
			if ( desc->descSeenFrame <= gFrameCounter )
				...
				if ( desc->descNameVisible > kName_Hidden )
					...
					desc->descSeenFrame = gFrameCounter + kNameDelay_Hide;	// for some time
					}
				// If it's been a "long" time, force the name to be drawn anyway
				if ( gFrameCounter - desc->descSeenFrame > kNameDelay_MemoryRefresh )
					desc->descSeenFrame = 0;	// reshow the guy, it's been long enough
				}
			}
...
# endif  // FAINT_MONSTERS
	...
	// v513: some mid-2007 iMacs fail to draw name-box backgrounds & frames if blending
	// is disabled, so we now turn it on throughout (up til drawing the actual text)
...
# if FAINT_MONSTERS
		glColor4us( frameColor.rgbRed, frameColor.rgbGreen, frameColor.rgbBlue, bgAlpha );
# else
		glColor3us( frameColor.rgbRed, frameColor.rgbGreen, frameColor.rgbBlue );
# endif
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			{
			// i could go through the motions and follow the codepath that is DrawBlendedName()
			// which protects me against future changes, but this is soooo much simpler...
...
# endif	// USE_OPENGL
			{
			DrawBlendedName( &box, desc->descNameVisible, backClr, textClr, dtsstyle, name );
			}
		}
	else
#endif  // AUTO_HIDENAME
...
# if ! NAME_BOX_FIX_513
			glColor3us( frameColor.rgbRed, frameColor.rgbGreen, frameColor.rgbBlue );
# else
			glColor4us( frameColor.rgbRed, frameColor.rgbGreen, frameColor.rgbBlue, 0xFFFF );
# endif
			}
		else
#endif	// USE_OPENGL
...
#if defined(USE_OPENGL) && defined(DEBUG_VERSION) && defined(OGL_SHOW_IMAGE_ID)
		// for finding problem images when using light casters
	ShowMessage( "mobile ID: %d, state: %d", (int) desc->descID, (int) mobile->dsmState );
#endif	// USE_OPENGL && DEBUG_VERSION && OGL_SHOW_IMAGE_ID
...
//	int halfSize   = mobileSize / 2;
	...
	&&   srcBox.rectBottom <= icBounds.rectBottom )
		...
		if ( gUsingOpenGL )
			...
			switch ( blitter )
				{
				case kPictDef75Blend: mapID = kIndexToAlphaMapAlpha25TransparentZero; break;
				case kPictDef50Blend: mapID = kIndexToAlphaMapAlpha50TransparentZero; break;
				case kPictDef25Blend: mapID = kIndexToAlphaMapAlpha75TransparentZero; break;
				
				default:
				case kPictDefNoBlend: mapID = kIndexToAlphaMapAlpha100TransparentZero; break;
				}
			drawOGLPixmapAsTexture(	srcBox, dstBox, ic->icImage.cliImage.GetRowBytes(),
...
// brain damage
		...
		if ( mobile->dsmIndex != 0 )
			ShowMessage( "Drawing descriptor %d '%s'.", mobile->dsmIndex, desc->descName );
#endif
		}
	else
		{
// brain damage
#if 0
		ShowMessage( " * Bad frame %d in picture %d.", state, (int) ic->icImage.cliPictDefID );
#endif
...
# else
			glRecti( dstBox.rectLeft, dstBox.rectTop, dstBox.rectRight, dstBox.rectBottom );
# endif  // 1
			}
		else
#endif	// USE_OPENGL
			{
			DTSOval oval;
			oval.Set( dstBox.rectLeft, dstBox.rectTop, dstBox.rectRight, dstBox.rectBottom );
			Paint( &oval );
			}
	...
	for(;;)
		...
		if ( kBubbleYell == bType )
			...
			if ( kBubblePosNone == table->descBubblePos )
				{
				table->descBubbleLoc.ptV = gLayout.layoFieldBox.rectTop  - gFieldCenter.ptV;
				table->descBubbleLoc.ptH = gLayout.layoFieldBox.rectLeft - gFieldCenter.ptH;
				}
			Draw1Bubble( table, kDrawBubbleNamePrefix );
			continue;
			}
...
{
#if TARGET_CPU_PPC
	// dunno why this is needed, but on my creaky old PowerBook G4 (OS 10.5), it is.
...
//	memset( buf, 0, height * rb );
	...
	// Is this right? what about dimmed or non-opaque bubbles? Or have they already
	// been properly handled, prior to this stage?
	disableTexturing();
	disableBlending();
	disableAlphaTest();
	
	// copy existing bubble bits into buffer
	int fieldHeight = gLayout.layoFieldBox.rectBottom - gLayout.layoFieldBox.rectTop;
	...
	for ( int row = height - 1; row >= 0; --row )
		{
		// we can stop swapping as soon as the src and dst pointers overtake each other
		if ( src > dst )
			break;
...
/*
**	IsLudic()
**	Is ludification appropriate, either in general or for this specific descriptor?
*/
static bool
IsLudic( const DescTable * desc )
{
	...
	&&	 (    ( desc->descID == 1261 || desc->descID == 1262 )
		   && ( desc->descMobile->dsmState == 7 || desc->descMobile->dsmState == 11 ) ) )
		{
		return true;
		}
...
// (why not in an initializer?) -- A. because GetTextWidth() requires a valid QDPort,
// which we cannot count on having until DoDraw() time.
	if ( not gUsingOpenGL
...
#ifdef USE_OPENGL
	GLfloat targetAlphaScale = 1.0f;	// this means use the alpha implied in the alpha map
	if ( gUsingOpenGL )
		{
# ifdef OGL_USE_TEXT_FADE
		if ( desc->descBubbleCounter <= kTextFadeLength )
			...
			||   kBlitterTransparent == gPrefsData.pdBubbleBlitter )
				...
				targetAlphaScale = (desc->descBubbleCounter + 1) * invFadeLengthPlusOne;
				}
			}
# endif	// OGL_USE_TEXT_FADE
		...
		switch ( blitter )
			...
			case kBlitter75:
				drawOGLPixmapAsTexture(	src, desc->descBubbleBox,
					cache->icImage.cliImage.GetRowBytes(), kIndexToAlphaMapAlpha25TransparentZero,
					TextureObject::kBalloonArray, cache, targetAlphaScale );
				break;
			
			case kBlitter50:
				drawOGLPixmapAsTexture(	src, desc->descBubbleBox,
					cache->icImage.cliImage.GetRowBytes(), kIndexToAlphaMapAlpha50TransparentZero,
					TextureObject::kBalloonArray, cache, targetAlphaScale );
				break;
			
			case kBlitter25:
				drawOGLPixmapAsTexture(	src, desc->descBubbleBox,
					cache->icImage.cliImage.GetRowBytes(), kIndexToAlphaMapAlpha75TransparentZero,
					TextureObject::kBalloonArray, cache, targetAlphaScale );
				break;
		...
		||	desc->descType != kDescPlayer )
			...
			// translucent for everything else
			SelectiveBlit( this, image, &src, &desc->descBubbleBox, gPrefsData.pdBubbleBlitter );
		}
...
# endif	// OGL_USE_TEXT_FADE
			...
			if ( text[ 0 ] )	// this is to fix the "option-space bug"
				...
				drawOGLText( dst.rectLeft, dst.rectTop + kTextAscent, geneva9NormalListBase,
...
# ifdef OGL_USE_TEXT_FADE
				...
				&&   targetAlphaScale < 1.0f )
					...
					glColor4us( sLangColors[bubColor].red, sLangColors[bubColor].green,
						sLangColors[bubColor].blue, targetAlphaScale * 0xFFFF );
					}
				else
...
# endif	// OGL_USE_TEXT_FADE
			...
			if (
				...
				( kTextLargeWidth == interiorWidth )
				 	&&
				(
					( kBubblePosUpperRight == desc->descBubblePos )	// should I use pos instead?
						||
					( kBubblePosLowerRight == desc->descBubblePos )	// should I use pos instead?
				)
			)
				{
				dst2.rectLeft -= 1;
			...
			// which means we can use shorts instead of floats (like that's a big deal anymore...)
			...
			// the round part of the round cornered rectangle
				...
				glBitmap( 0, 0, 0, 0,
					...
					-(dst2.rectBottom - gFieldCenter.ptV - 5 + 0.5f), // -v cuz of glScale(1, -1)
					nullptr );
				
					// lower left
				glBitmap( 4, 4, 4, 4, 0, dst2.rectBottom - dst2.rectTop - 4, corner );	// ditto
				
					// upper left
				glPixelStorei( GL_UNPACK_SKIP_ROWS, 4 );
				glBitmap( 4, 4, 4, 4, dst2.rectRight - dst2.rectLeft - 4 , 0, corner );	// ditto
				
					// upper right
				glPixelStorei( GL_UNPACK_SKIP_PIXELS, 4 );
				glBitmap( 4, 4, 4, 4, 0, - (dst2.rectBottom - dst2.rectTop - 4), corner );	// -v
				...
				{
				DTSRound innards;
				innards.Set( dst2.rectLeft, dst2.rectTop, dst2.rectRight, dst2.rectBottom );
...
*/
void
PositionBubble( DTSRect * dst, const DTSImage * image, const DescTable * desc, int pos )
{
...
ChooseBubblePosition( const DTSImage * image, DescTable * desc )
{
	...
	int mobFacing = desc->descMobile ? desc->descMobile->dsmState / 4 : kFacingSouth;
	switch ( mobFacing )
		{
...
CalcBubblePosValue( const DTSImage * image, DescTable * desc, int pos )
{
	...
	for ( int count = gNumMobiles;  count > 0;  --count, ++dsm )
		...
		if ( not ic )
			{
			DTSPoint loc;
			loc.Set( dsm->dsmHorz + gFieldCenter.ptH, dsm->dsmVert + gFieldCenter.ptV );
...
void
TransludentBarView::Draw1TranslucentBar( int mValue, int mMax, const DTSColor * mColor,
	ushort alpha, const DTSRect& viewBounds )
{
# endif	// USE_OPENGL
...
# endif	// USE_OPENGL
	...
	if ( HB_Color_Variable == ( gPrefsData.pdHBColorStyle & 0x0FFFF ) )
		{
		// Robert's style:
		// draw the full part in color according to the value
		// experimentally determined to simulate the nametag color as server sends 'em
		if ( mValue >= 228 )
			{
...
# endif	// USE_OPENGL
		...
		{
		// Michel's style: bars have individual colors
		// (the view is drawn once before being fully set, so protect against that)
		if ( mColor )
			{
# ifdef USE_OPENGL
			if ( gUsingOpenGL )
				glColor4us( mColor->rgbRed, mColor->rgbGreen, mColor->rgbBlue, alpha );
			else
# endif	// USE_OPENGL
				SetForeColor( mColor );
			}
...
# ifdef USE_OPENGL
	if ( gUsingOpenGL )
		glRecti( bounds.rectLeft, bounds.rectTop, bounds.rectRight, bounds.rectBottom );
	else
# endif	// USE_OPENGL
		Paint( &bounds );
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			...
			glVertex2f( bar.rectLeft + width * mValue / 255 + 0.5f, bar.rectTop + 0.5f );
			glVertex2f( bar.rectLeft + width * mValue / 255 + 0.5f, bar.rectBottom + 0.5f );
			}
		else
# endif	// USE_OPENGL
			{
	...
	if ( mMax != 0 && mMax != mValue && mMax != 255 )	// original was: if ( mMax != mValue )
		{
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			{
			...
			glVertex2f( bar.rectLeft + width * mMax / 255 + 0.5f, bar.rectTop + 0.5f );
			glVertex2f( bar.rectLeft + width * mMax / 255 + 0.5f, bar.rectBottom + 0.5f );
			}
		else
# endif	// USE_OPENGL
			{
...
# ifdef USE_OPENGL
	if ( gUsingOpenGL )
		...
		if ( insideBeginEnd )
			{
			glEnd();
				// this isn't needed here; included in case function gets expanded.
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			bar.Set( 0, 0,
					gLayout.layoHPBarBox.rectRight - gLayout.layoHPBarBox.rectLeft,
					gLayout.layoHPBarBox.rectBottom - gLayout.layoHPBarBox.rectTop + 2 );
		else
# endif	// USE_OPENGL
		...
		switch ( gPrefsData.pdHBPlacement )
			...
			case HB_Placement_Bottom:
				// Michel's style: bars spread across bottom
				
				// figure left-over space after 3 instances of the bar
				deltah = (field.rectRight - field.rectLeft) - 3 * barWidth;
				
				// now apportion that space into 6 slots, one on each side of the bars
				deltah /= 6;
				...
				// each bar is offset from the last by its width plus 2/6 of the unused space
			...
			case HB_Placement_LowerLeft:
				// Robert's style: put the bars in the lower-left
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			offStatusBarView.Draw1TranslucentBar( gFrame->mHP, gFrame->mHPMax, &barColors[0],
				gFrame->mHP < kAlphaTrigger ? kAlpha75 : kAlpha50, bar) ;
		else
# endif	// USE_OPENGL
			{
			offStatusBarView.DrawTransBar(gFrame->mHP, gFrame->mHPMax, &barColors[0] );
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			offStatusBarView.Draw1TranslucentBar( gFrame->mBalance, gFrame->mBalanceMax,
				&barColors[1], gFrame->mBalance < kAlphaTrigger ? kAlpha75 : kAlpha50, bar );
		else
# endif	// USE_OPENGL
			{
			offStatusBarView.DrawTransBar(gFrame->mBalance, gFrame->mBalanceMax, &barColors[1]);
			SelectiveBlit( this, &offStatusBar, &offStatusBarBounds, &bar,
...
# ifdef USE_OPENGL
		if ( gUsingOpenGL )
			offStatusBarView.Draw1TranslucentBar( gFrame->mSP, gFrame->mSPMax, &barColors[2],
				gFrame->mSP < kAlphaTrigger ? kAlpha75 : kAlpha50, bar );
		else
# endif	// USE_OPENGL
			{
			offStatusBarView.DrawTransBar(gFrame->mSP, gFrame->mSPMax, &barColors[2] );
			SelectiveBlit( this, &offStatusBar, &offStatusBarBounds, &bar,
...
CLOffView::DrawTranslucentHandObjects()
{
	...
	if ( id )
		...
		if ( const ImageCache * cache = CachePicture( id ) )
			...
			itemBox.Move( gLayout.layoFieldBox.rectLeft + 16, gLayout.layoFieldBox.rectTop + 16 );
			SelectiveBlit( this, &cache->icImage.cliImage, &src, &itemBox, kBlitter50 );
			}
		}
	...
	if ( id )
		...
		if ( const ImageCache * cache = CachePicture( id ) )
			...
			DTSRect src = cache->icImage.cliImage.imageBounds;
			src.rectBottom = src.rectTop + cache->icHeight;
			Draw( &cache->icImage.cliImage, kImageCopyFast, &src, &gLayout.layoRightObjectBox );
...
CLOffView::DrawHandObjects()
{
	...
	if ( cache )
		...
		src.rectBottom = src.rectTop + cache->icHeight;
		Draw( &cache->icImage.cliImage, kImageCopyFast, &src, &gLayout.layoLeftObjectBox );
	...
	if ( offMovieSignShown )
		...
		if ( gUsingOpenGL )
			...
			drawOGLText( gLayout.layoFieldBox.rectLeft + 5,
					gLayout.layoFieldBox.rectTop + 5 + kTextAscent, geneva9BoldListBase, text );
			}
		else
#endif  // USE_OPENGL
			{
...
{
	Init( kCLDepth );
	DTSError result = tvImage.Init( nullptr, kTextViewWidth, kTextViewHeight, kCLDepth );
	if ( noErr == result )
...
InitLayout()
{
	...
	if ( noErr == result )
		...
		SwapEndian( gLayout );
#endif
		gFieldCenter.ptV = (gLayout.layoFieldBox.rectTop + gLayout.layoFieldBox.rectBottom) / 2;
		gFieldCenter.ptH = (gLayout.layoFieldBox.rectLeft + gLayout.layoFieldBox.rectRight) / 2;
		}
...
ExtractFramePictures( const CCLFrame * ds )
{
	...
	if ( gPrefsData.pdShowImageCount )
		ShowMessage( "Drawing %d/%d images.", (int) ds->mNumPict, (int) ds->mNumPictAgain );
...
# ifdef CL_DO_WEATHER
	...
	if ( weatherImg )
		...
		if ( not ( gWeatherInfo.GetOutdoorOnly() && (gFrame->mLightFlags & kLightNoShadows) ) )
...
ExtractFrameMobiles( const CCLFrame * ds )
{
	...
	if ( newme && newme != gThisPlayer )
		...
		// will cycle back and forth every frame.
			...
			// Do I leave him alone? or unfriend him? Or maybe
			// remember his old friendship state and restore that? (gets expensive)
			if ( gThisPlayer )
...
ExtractStateData( const CCLFrame * ds, int lastAckFrame, int receivedResend )
{
	...
	if ( lastAckFrame != 0 )
		...
		if ( not CCLMovie::IsReading() )
			...
			ShowMessage( "can this even happen? last ack %d ack %d resend %d gResend %d.",
	...
	switch ( gStateMode )
		...
		for(;;)
			{
			// there's a slim but non-zero chance that the two bytes that make up
			// the state-size word end up in different UDP packets, so we have to
			...
			case kStateSizeLo:		// grab lo-order byte of ditto
				...
				||   gStateExpectSize < 4 )
					{
					ShowMessage( "StateData: expected size possibly bad: %d.", gStateExpectSize );
					}
				FALLTHRU_OK;
			
			case kStateData:	// we have the complete size word, so we know how much more
				{				// data to expect in this frame
...
#ifdef HANDLE_MUSICOMMANDS
			if ( gTuneQueue.HandleCommand( start, len ) )	// do nothing, it's handled
				break;
#endif
			
#if ENABLE_MUSIC_FILES			
			if ( CheckMusicCommand( start, len ) )			// scripted song change: ditto
				break;
#endif
...
ExtractTypeThinkBubble( DescTable * target )
{
	...
	// but check first the whole BEPP string would fit
	if ( pos + strlen(kBEPPThinkBubble_think) >= strlen(text) )
		return kBubbleThink_none;
	
	BufferToStringSafe( bepptype, sizeof bepptype, text + pos, sizeof kBEPPThinkBubble_think );
	
	// match the extracted BEPPstr with our predefined think types
	if ( not strncmp( bepptype, kBEPPThinkBubble_think, sizeof kBEPPThinkBubble_think ) )
		thinktype = kBubbleThink_think;
	if ( not strncmp( bepptype, kBEPPThinkBubble_thinkto, sizeof kBEPPThinkBubble_thinkto ) )
		thinktype = kBubbleThink_thinkto;
	if ( not strncmp( bepptype, kBEPPThinkBubble_thinkclan, sizeof kBEPPThinkBubble_thinkclan ) )
		thinktype = kBubbleThink_thinkclan;
	if ( not strncmp( bepptype, kBEPPThinkBubble_thinkgroup,
				sizeof kBEPPThinkBubble_thinkgroup ) )
		{
		thinktype = kBubbleThink_thinkgroup;
		}
	
	// we saved the type of the think, now we can remove that extra piece of information from
	// the original bubble text (by copying the text backwards over the bepp string)
...
**		ubyte+	text of bubble, zero-terminated, using MacRoman character encoding.
**				(may be zero-length; see note below).
**
**	Notes: The server sends no bubble text if you don't understand the language (to
**	prevent rogue clients from eavesdropping on conversations they shouldn't). In that
**	case, the client must synthesize an appropriate replacement. On the other hand,
**	even though you might not understand, say, kLanguageSanskrit, you should still be
**	able to distinguish between a monosyllabic grunt and a major oration! Hence the
**	bubble size code flags: the client can use those to choose the proper bubble image
**	and a suitable synthetic content string -- see FillUnknownLanguage().
**
**	Some players may have lost the ability to understand the Common language, e.g. for
**	RP reasons. So that GMs can nevertheless communicate reliably, we added the Standard
**	language, which is implicitly understood by everyone, always.
**
**	This routine needs to do two main jobs: first, obviously, to cause the correct
**	visual bubble image to appear in the playfield, and second, to generate the
**	textual transcript for the text window. The first task is downright easy in
...
Handle1Bubble( const uchar * ptr )
{
	...
	// (it may be time to retire some of these fixups; v142 was a *long* time ago...)
	if ( CCLMovie::IsReading() )
		{
...
//				newtype |= kBubbleNormal;	// no-op
	...
	if ( lang != 0 )
		{
		// the movie-patching code has determined the language - adjust the type
	...
	if ( type == kBubbleThought )
		...
		int minAge = INT_MAX;
		for ( int step = kDescTableBaseSize;  step < kDescTableSize;  ++step, ++thought )
			{
	...
	if ( text )
		StringCopySafeNoDupSpaces( target->descBubbleText, text, sizeof target->descBubbleText );
	else
		{
#if USE_WHISPER_YELL_LANGUAGE
		FillUnknownLanguage( type, lang, target->descName, target->descBubbleText );
#else
	...
	if ( speakerName[ 0 ] == '\0' )
		...
		switch ( target->descType )
			{
			case kDescMonster:	speakerName = _(TXTCL_BUBBLE_UNKNOWN_MONSTER);		break;
			case kDescPlayer:	speakerName = _(TXTCL_BUBBLE_UNKNOWN_PLAYER);		break;
			default:			speakerName = _(TXTCL_BUBBLE_UNKNOWN_SOMETHING);	break;
			}
		}
	...
	switch ( type2 )
		...
		case kBubbleNormal:
			{
			// Person says, "message"
			char punct = target->descBubbleText[ strlen( target->descBubbleText ) - 1];
			const char * verb;
			switch ( punct )
				{
		...
		case kBubbleYell:
			...
			if ( 0 == (type & kBubbleNotCommon) )
				...
				if ( (kBubbleCommon + 1) == langindex )
						...
						But there's still a problem with people who don't understand common.
						*/
					snprintf( buff, sizeof buff,
//						"%s %s, \"%s\"",
						//"%s yells, \"%s\"",
...
//						gVerbTable[kLanguageLogYellVerb][langindex],
		...
		case kBubbleThought:
			...
			if ( not ptr2 )
				...
				if (
					...
					snprintf( buff, sizeof buff, _(TXTCL_BUBBLE_THINK), target->descName, ptr2 );
					}
				}
			}
			break;
		...
		case kBubbleNarrate:
			// (person): message
			snprintf( buff, sizeof buff, "(%s): %s", speakerName, target->descBubbleText );
			break;
...
#if USE_STYLED_TEXT
	int friends;
	// I would much prefer to test for target == gThisPlayer, but that will be
	// the wrong test if it's a thought bubble. The risk of doing it this way is
	// that gThisPlayer might not be set, although I don't see how that can happen.
	
	// Argh. turns out it CAN happen... during the first few frames, & rarely at
	// that. But when it does, it's a crasher.
	if ( gThisPlayer )
		{
	...
	// appear in the text window and log. But double-ignored players' bubbles
	// are suppressed altogether. Set this bool to false to log them, nevertheless.
...
HandleInvCmdFull( const uchar * ptr )
{
	...
	int shift = 0;
	int index;
	for ( index = (itemcount + 7) >> 3;  index > 0;  --index )
		{
		equips |= *ptr++ << shift;
		shift += 8;
		}
	...
	for ( index = 1;  index <= itemcount;  ++index )
		...
		// index must be set to 0 for non-template items
...
#ifdef DEBUG_VERSION
//	ShowMessage( "Receiving custom name '%s' for %d/%d.", text, (int) id, index );
#endif
...
//	const char * prefix = nullptr;
	...
	// I had to move this here, from PlayersWin.cp, since these messages have
	// no BEPPrefix
	if ( '[' == *text || 0xA5 == * (const uchar *) text || '*' == *text )	// A5 is bullet
	...
	//
	// Michel: get some special messages, to be handled as default anyway
	//
	...
	// remove the player name and monster name delimiters now that ParseInfoText has seen 'em
	const char * src = text;
	...
	&&	 text
	&&	 ( '*' == *text  ||  '[' == *text  ||  0xA5 == * (const uchar *) text ) )	// bullet
		{
		CLStyleRecord temp;
	...
	if ( TFFormat * format = win->winTextField.GetFormat() )
		...
		if ( numLines > maxLines )
			{
			win->winTextField.SelectText( 0, format->tfLineStarts[ numLines - maxLines ] );
...
CheckWeather( const char * inCmd, size_t inLen )
{
	...
	if ( inLen >= 4 + 1		// ensure there's at least one char of actual data (after '/wt ')
	&&	 inCmd
...
#ifdef DEBUG_VERSION
//			ShowMessage( "setting weather to img: %d, outdoorsonly: %d", image, outdooronly );
#endif // DEBUG_VERSION
			return true;
			}
		}
...
*/
void
StringCopySafeNoDupSpaces( char * orgdst, const char * inSrc, size_t size )
{
	bool bLastWasSpace = true;
	char * dst = orgdst;
	char * limit = dst + size - 1;
...
**	DoNotification()
**
**	provide a MacOS Notification upon receipt of a think-to, group think, or clan think
*/
void
DoNotification( const char * senderName, const char * format, const char * thought )
{
	// If it's a friend, and the application is not in front, and we receive a thinkto,
	// show up a notification.
	...
	// but only give simple message if you are sitting or dead
	if ( kPoseSit  == state
	||	 kPoseDead == state )
		{
	...
	// remove any previous one, ignore error. it's to prevent a crash.
	if ( gNotificationActive )
		err = ::NMRemove( &gNotif );
	...
	// under OS X (thru 10.1.3, anyway), this causes the message to be written to the
	// console log, for no good reason that I can see. It's not at all clear how to
	// prevent that, either.
...
#endif	// USE_WHISPER_YELL_LANGUAGE
{
	...
	if ( lll < 0  ||  lll >= kNumLanguages )
		...
		// kLanguageNormalVerb, kLanguageWhisperVerb, kLanguageUnknownYellVerbFirst = 0, 1, 2
	...
	switch ( lang & kBubbleCodeMask )
		...
		case kBubbleUnknownMedium:
			snprintf( dest, kMaxDescTextLength, _(TXTCL_BUBBLE_UNKNOWN_MEDIUM_IN_LANG),
				name, verbs, gLanguageTableShort[lll] );
			break;
		
		case kBubbleUnknownLong:
			snprintf( dest, kMaxDescTextLength, _(TXTCL_BUBBLE_UNKNOWN_LONG_IN_LANG),
...
{
	// i HATE doing stuff like this, but I want to minimize the changes i make to base classes
	CLWindow * win = static_cast<CLWindow *>( gGameWindow );
...
CLOffView::CreateOGLFontDisplayLists( AGLContext ctx )
{
	...
	geneva9NormalListBase = Make1FontList( ctx, fontID, fsize, normal );
	geneva9BoldListBase = Make1FontList( ctx, fontID, fsize, bold );
	geneva9ItalicListBase = Make1FontList( ctx, fontID, fsize, italic );
	geneva9BoldItalicListBase = Make1FontList( ctx, fontID, fsize, bold | italic );
	geneva9UnderlineListBase = Make1FontList( ctx, fontID, fsize, underline );
	geneva9BoldUnderlineListBase = Make1FontList( ctx, fontID, fsize, bold | underline );
	geneva9ItalicUnderlineListBase = Make1FontList( ctx, fontID, fsize, italic | underline );
	geneva9BoldItalicUnderlineListBase = Make1FontList( ctx, fontID, fsize,
...
CLOffView::DeleteOGLObjects()
{
	...
	bindTexture2D( 0 );
	bindTexture( 0 );
	glDeleteTextures( 1, &gOGLNightTexture );	// note bindTexture immediately preceeding
	gOGLNightTexture = 0;				// zero is not a valid object name
	gOGLNightTexturePriority = -1.0f;	// -1 is not a valid priority
	glDeleteLists( gLightList, 1 );
	gLightList = 0;
...
initOGLIndexToColorTables( ushort * i2r, ushort * i2g, ushort * i2b, GLint mapsize )
{
	...
	for ( int i = 0;  i < mapsize;  i += 8 )
		...
		ShowMessage("\t$\"%4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x\"",
			i2r[i], i2r[i+1], i2r[i+2], i2r[i+3], i2r[i+4], i2r[i+5], i2r[i+6], i2r[i+7] );
		}
	...
	for ( int i = 0;  i < mapsize;  i += 8 )
		{
		ShowMessage("\t$\"%4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x\"",
			i2g[i], i2g[i+1], i2g[i+2], i2g[i+3], i2g[i+4], i2g[i+5], i2g[i+6], i2g[i+7]);
		}
	...
	for ( int i = 0;  i < mapsize;  i += 8 )
		{
		ShowMessage("\t$\"%4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x %4.4x\"",
			i2b[i], i2b[i+1], i2b[i+2], i2b[i+3], i2b[i+4], i2b[i+5], i2b[i+6], i2b[i+7]);
		}
}
#endif	// 0
...
countLightDarkCasters()
{
	...
	for ( ; numMobiles > 0 ;  --numMobiles )
		...
		if ( DescTable * desc = gDescTable + mobile->dsmIndex )
			...
			if ( cache && cache->icImage.cliPictDef.pdFlags & kPictDefFlagEmitsLight )
				{
				int state = mobile->dsmState;
				if ( not (cache->icImage.cliPictDef.pdFlags & kPictDefFlagOnlyAttackPosesLit)
	...
	ShowMessage( "found %d lightcasters, %d darkcasters", gNumLightCasters, gNumDarkCasters );
...
//		ushort redshiftus = redshiftClearColorf * 0xFFFF;
		...
		if ( gUsingOpenGL )
			...
			glBlendFunc( GL_ONE, GL_ONE );	// we want the light combining everywhere
							// well, not anymore, but... see below about blend subtract
			if ( gOGLUseDither )
				glEnable( GL_DITHER );
...
//			bufferIsInverted = false;	// default settings
			...
			{
// hmmm.
// not using one of the DTS constructs, so I can't use SetForeColor, Paint, SetBackColor
// should I use native OS calls, or try to find the closest dts class and use that?
			::GetGWorld( &oldCGrafPtr, &oldGDHandle );
			::SetGWorld( gLightmapGWorld, nullptr );
			PixMapHandle pmh = ::GetGWorldPixMap ( gLightmapGWorld );
			::LockPixels( pmh );
				// the gworld was not created with the pixPurge flag, so it can't be purged,
				//  so LockPixels can't fail (right?)
		...
		//
		// for night from 0.0-0.5, it's not used at all.  this helps avoid saturation, and lets
		// the more subtle special effects like healer pulses have more of a chance of being seen.
		// the (current) nominal exile pseudo-torch is 0.5 intensity, so the light is
		// at 1.0 (saturated) at the exile.
		//
		// for night 0.5-1.0, we'll put in a scaled down (night - 0.5) version of the old effect,
		// so that the ambient light right at the exile is remains 0.5 regardless of nightlevel.
		// combined with the pseudo-torch, each exile will see themselves fully lit,
		// and other exiles at not less than 0.5 (not counting the effect of darkcasters)
		//
		// if the overall night level is determined by ambient moonlight, then i'm tempted to call
		// this secondary effect starlight, since like real starlight it's largly overwhelmed by
		// a bright moon, but is the primary light when there is little or no moon.
		//
		// which all sounds plausible, except that it isn't affected by being inside a cave
		// or similar location where starlight wouldn't reach.  oh well.
		if ( clearColorf < 0.5f )
			{
			GLfloat starlightColorf = 0.5f - clearColorf;
		...
		// which scaled each light according to the remaining "range" between full daylight (1.0)
		// and the ambient light level (nightlevel).  this gave a smoother light effect.
		// this doesn't make sense with darkcasters, which would need to be scaled according to
		// the difference between ambient and full dark, and neither lightcasters nor darkcasters
		// will want to be scaled differently than the "other" guys.  so we will now be unscaled
		// (full strength), which give light and dark full leverage against each other, at the
		// cost of more saturation. this means the light around an exile or monster is more
		// likely to reach the limits of [0..1] and get clamped, and giving an optical illusion
		// of a ring around them at the point where the light/dark stops saturating ("mach line")
	...
	//
	// mathematically, I want D-S (destination==lightmap minus source==darkcaster)
	// so first I use GL_ONE_MINUS_DST_COLOR * an all white source rectangle, so the
	// lightmap changes from D to (1-D).  then I draw the darkcasters as light; more
	// "darkness" is drawn as more white.  thus the lightmap contains (1-D+S).
	// finally, i flip it again, so I get 1-(1-D+S),  which is just (D-S)
	//
	// yes, it sounds expensive to flip the entire framebuffer TWICE, but it isn't
	// too bad in practice, at the window size we use:  drawtimes on a
	// 800MHz G4 with Radeon 7500 went from 6ms to 10ms.
	// note also that we only have to do this on the Rage128, Radeon, and Radeon 7500.
	// everything else (including, amazingly, the software renderer) supports blend subtract
	...
	for ( int v = startV; v < endV; ++v )
		...
		for ( int h = startH; h < endH; ++h )
			...
			if ( colorScale )
				...
				if ( lightdarkcaster )
					{
					if ( oldColor != 0x00ffffff )	// if  not already fully saturated (white)
						{
...
{
// keep here, or make external, so it can be deleted in deleteNonOGLObjects() ?
// we use this circle a lot, so we'll give it it's own optimal-size array
// and eliminate the index scaling
	...
	int rowBytes = ::GetPixRowBytes( pmh ) & 0x3fff;	// carbon + 8.5; will this be a problem?
	...
	for ( int v = centerV; v < endV; ++v )
		...
		for ( int h = centerH; h < endH; ++h )
			...
			if ( colorChanged )
				...
				char * ll = baseAddr + v * rowBytes + ( startH + endH - h - 1 ) * bytesPerPixel;
				...
				* reinterpret_cast<int32_t *>( ul ) = newColor;
				char * ur = baseAddr + ( startV + endV - v - 1 ) * rowBytes + h * bytesPerPixel;
...
// to do: find a better (common, once-only) place for this
	...
	if ( not gRGB444ToIndexMap )
		...
		GetAppData( 'BTbl', 8, reinterpret_cast<void **>( &gRGB444ToIndexMap ), nullptr );
#else
		// pixel needs to be in bounds
		uchar * address = reinterpret_cast<uchar *>( baseAddr ) + startV * rowBytes + startH;
			...
			for ( ushort g = 0; g < 16; ++g )
				...
				for ( ushort b = 0; b < 16; ++b )
					...
					if ( i2cIndex != gRGB444ToIndexMap[ index ] )
						{
						ShowMessage(
							"unexpected color mismatch when generating gRGB444ToIndexMap: "
							"%d != %d (%x, %x, %x(",
							i2cIndex, gRGB444ToIndexMap[ index ], c.red, c.green, c.blue );
	...
	for ( int v = startV; v < endV; ++v )
		...
		for ( int h = startH; h < endH; ++h )
			...
			ulong lightmapColor = * lightmapPtr;
				...
				// could be slightly faster (use add instead of multiply)
			uchar * destPtr = reinterpret_cast<uchar *>( baseAddr ) + v * rowBytes + h;
...
//		case 2227:	// spear
//		case 2230:	// arrow
			// these appear to be flaming in some poses.  with the old way, we just checked
			// for the correct pose in the big ugly switch. With the new way, there is no
			// way to do that without reintroducing the big ugly switch for the special cases.
			// suggestion:  create separate "flaming arrow" and "flaming spear" images, which
			// makes the image file slightly bigger, but simplifies the drawing code
			// by avoiding the switch.
