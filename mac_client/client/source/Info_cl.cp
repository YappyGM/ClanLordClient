...
# else
//#  define	kTestServerAddress	"server.clanlord DOT com" TEST_PORT	// new test host 18 Aug 2004
#  define	kTestServerAddress	"server.deltatao.com" TEST_PORT	// DNS change 27 July 2014
# endif
#endif
...
**
**	This class requires that the underlying input field be a modern EditUnicodeText
**	control, not an old-fashioned EditText one. The E-U-T control can be either the
**	plain or the password variant -- we don't care.
**
**	The warning icon (if there is one) will be shown or hidden dynamically, depending on
**	whether the field's contents are fit for purpose (as determined by the Acceptable()
**	method, which subclasses should override).
*/
class RestrictEdit
{
protected:
...
public:
	...
	void			SetText( const char * newText ) const __attribute__(( __nonnull__ ));
...
private:
	...
	virtual bool	Acceptable() const { return true; }	// is text valid for intended use?
...
{
public:
	void			Init( HIViewRef item, HIViewRef icon, const char * initialText );
	virtual bool	Acceptable() const;		// is the current contents OK as a CL password?
};
...
public:
	void			Init( HIViewRef item, HIViewRef icon, const char * initialText );
	virtual bool	Acceptable() const;		// is the current contents OK as a name?
};
...
class SelectAccountDlg : public HIDialog
{
	...
	virtual CFStringRef	GetNibFileName() const { return CFSTR("Select Account"); }
//	virtual CFStringRef	GetPositionPrefKey() const { return CFSTR("SelectAccountDlgPosition"); }
...
class CharacterList : public BrowserList
{
	...
	virtual void		DoDoubleClick( DataBrowserItemID item, DataBrowserItemDataRef data );
...
public:
	...
	void				ParseLogMessage( const Message& msg, int& ourIdx, int& maxChars );
...
class CharacterManagerDlg : public HIDialog, public GameTickler
{
	...
	static HIViewID Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Character Manager"); }
	virtual CFStringRef	GetPositionPrefKey() const { return CFSTR("CharacterManagerDlgPosition"); }
...
public:
	...
	HIViewRef			GetSpinnerView() const { return GetItem( Item( cmdSpinner ) ); }
...
class CreateCharacterDlg : public HIDialog
{
	...
	static HIViewID Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Create Character"); }
//	virtual CFStringRef	GetPositionPrefKey() const { return CFSTR("CreateCharacterDlgPosition"); }
...
**
**	requests confirmation before deleting the currently selected character, via CharMgr
*/
class DeleteCharacterDlg : public HIDialog
{
	static const OSType Signature = 'DelC';
	static HIViewID Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Delete Character"); }
...
private:
	...
	static HIViewID		Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Select Character"); }
...
class EnterPasswordDlg : public HIDialog
{
	...
	static HIViewID		Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Enter Password"); }
...
**
**	a bit like EnterPasswordDlg, but used only when confirming a character transfer
*/
class CharacterTransferPasswordDlg : public HIDialog
{
	static const OSType Signature = 'CTPw';		// "Character Transfer"
	static HIViewID		Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Character Transfer"); }
...
**
**	handles the task of requesting a password change for a character (from Char Mgr dlg)
*/
class ChangePasswordDlg : public HIDialog
{
	static const OSType Signature = 'CPwd';
	static HIViewID		Item( int n ) { return (HIViewID) { Signature, n }; }
	virtual CFStringRef GetNibFileName() const { return CFSTR("Change Password"); }
...
**	class ProxyDlg
**
**	A dialog to allow the user to specify an alternate address for the server.
**	(Yes, it's somewhat misnamed.)  The debug version also provides shortcut access
**	to 'localhost' and the Delta Tao test server.
*/
class ProxyDlg : public HIDialog, public GameTickler
{
...
#ifdef DEBUG_VERSION
	// Interface Builder doesn't like having multiple views with the exact same
	// controlIDs and signatures in the same nib file. So we subtly tweak the signatures
	// in the debug dialog. Sigh.
	...
	virtual CFStringRef	GetNibFileName() const { return CFSTR("Set IP Address"); }
	virtual CFStringRef	GetNibWindowName() const;
	virtual CFStringRef	GetPositionPrefKey() const { return CFSTR("ServerAddressDlgPosition"); }
...
static DTSError	SendWaitReplyLoop( char * challenge, Message * msg, size_t size,
...
RestrictEdit::Init( HIViewRef vu, CFStringRef validChars,
					  CFIndex maxLen, const char * iniText, HIViewRef icon /* = nullptr */ )
{
	// stash this stuff
	reControl = vu;
	reMaxLen = maxLen;
	...
	if ( CFCharacterSetRef validSet = CFCharacterSetCreateWithCharactersInString(
		...
		{
		reInvalidChars = CFCharacterSetCreateInvertedSet( kCFAllocatorDefault, validSet );
		CFRelease( validSet );
		}
	...
	// don't need to worry about kEventTextDidChange if we have no associated warning icon
	if ( not icon )
		--nEvts;
	
	__Verify_noErr( InstallControlEventHandler( vu, RestrictEdit::HandleTextEvent,
...
*/
void
RestrictEdit::GetText( size_t dstLen, char * dst, CFStringEncoding enc /* = macRoman */ ) const
{
...
**	unlike other similarly-named methods, this one doesn't shift focus to the
**	input field (but maybe it ought to?)
*/
void
...
RestrictEdit::CreateCleanCopy( CFStringRef candidate ) const
{
	...
	CFStringRef result = CFStringCreateWithCharacters( kCFAllocatorDefault, bp, cleanLen );
...
{
	// paranoia: if no valid chars were provided to Init(), then we can't accept -any- input
	...
	if ( noErr == err )
		...
		if ( newText && (ntLen = CFStringGetLength( newText )) > 0 )
			...
			if ( ntLen > reMaxLen )
				{
				if ( CFStringRef trimmed = CFStringCreateWithSubstring( kCFAllocatorDefault,
											cleaned, CFRangeMake( 0, reMaxLen ) ) )
					{
					CFRelease( cleaned );
					cleaned = trimmed;
					}
			...
			if ( cleaned )
				...
				if ( CFStringGetLength( cleaned ) > 0 )
					{
					/* err = */ evt.SetParameter( kEventParamReplacementText, typeCFStringRef,
							sizeof cleaned, &cleaned );
					}
...
{
	RestrictEdit::Init( item, CFSTR(VALID_PASSWORD_CHARS), kMaxPassLen, initialText, icon );
}
...
{
	RestrictEdit::Init( item, CFSTR(VALID_NAME_CHARS), kMaxJoinLen, initialText, icon );
}
...
/*
**	DoJoinGame()
**	initiate the process of joining the game, from the Character Manager or Select Char dialogs
*/
static void
DoJoinGame()
{
#if 1
	// post a simulated "mJoinGame" menu event, in a deferred way so that
	// the dialog can finish unwinding before we plunge into that can o' worms.
	...
	joinEvt.SetParameter( kEventParamDirectObject, typeHICommand, sizeof cmd, &cmd );
	const UInt32 mCtxt = kMenuContextMenuBar | kMenuContextPullDown;
	joinEvt.SetParameter( kEventParamMenuContext, typeUInt32, sizeof mCtxt, &mCtxt );
...
SelectCharacter()
{
	...
	if ( noErr == result )
		...
		size_t size = dst - lo.logData;
		SimpleEncrypt( lo.logData, size );
		
		size = dst - reinterpret_cast<char *>( &lo );
		result = SendWaitReplyLoop( challenge, managerDlg.GetMessage(), size );
		}
	if ( noErr == result )
		result = BigToNativeEndian( lo.logResult );
...
//	bool bSavePass = gPrefsData.pdSavePassword;
	...
	sadAcctPass.Init( GetItem( Item(sadAcctPassEdit) ), icon, gPrefsData.pdAcctPass );
...
SelectAccountDlg::FieldDidChangeText( HIViewRef vu )
{
	...
	if ( vu == nameView )
		...
		if ( noErr == KeychainAccess::Get( kPassType_Account, name, pass, sizeof pass )
		&&   pass[ 0 ] )
			{
...
CharacterList::Init( CharacterManagerDlg * dlg, HIViewRef vu )
{
	...
	if ( noErr == err )
		...
		const EventTypeSpec evts = { kEventClassControl, kEventControlBoundsChanged };
		__Verify_noErr( InstallControlEventHandler( vu, DoEvent, 1, &evts, this, nullptr ) );
		}
...
CharacterList::PopulateList()
{
	...
	OSStatus err = AddDBItems( kDataBrowserNoItem, nItems, nullptr, prop_CharName );
	return err;
}
...
**	into the 'mChars' array.  Also returns the index into that array of our own character,
**	so we can select it; as well as the number of character slots for this account
	...
	if ( kEventClassControl == evt.Class() )
		...
		switch ( evt.Kind() )
			...
			case kEventControlBoundsChanged:
				{
				UInt32 atts( 0 );
				evt.GetParameter( kEventParamAttributes, typeUInt32, sizeof atts, &atts );
...
PackedDateToString( int paidup, char * buff, size_t maxlen )
{
	...
	if ( CFCalendarRef cal = CFCalendarCreateWithIdentifier( kCFAllocatorDefault,
		...
		if ( not CFCalendarComposeAbsoluteTime( cal, &paidUpTime, "yMd", year, month, day ) )
			{
			// we're hosed
			CFRelease( cal );
			return;
	...
	if ( fmtr )
		{
		paidUpStr = CFDateFormatterCreateStringWithAbsoluteTime( kCFAllocatorDefault,
						fmtr, paidUpTime );
		CFRelease( fmtr );
		}
	...
	if ( paidUpStr )
		...
		if ( not CFStringGetCString( paidUpStr, buff, maxlen, kCFStringEncodingMacRoman ) )
...
CharacterManagerDlg::Init()
{
	...
	if ( cmdIsDemo )
		...
		snprintf( buff, sizeof buff, _(TXTCL_CHARS_FOR_ACCOUNT), gPrefsData.pdAccount );
		SetText( Item(cmdAccountText), buff );
		}
	
	// extract account information
	const int32_t * logData = reinterpret_cast<int32_t *>( &cmdMsg.msgLogOn.logData );
...
#ifdef ARINDAL
		if ( kAcctStatusDemoAcct == status )
			ptr += snprintfx( buff, sizeof buff, _(TXTCL_PAID_X_DEMOCHARS), maxchars );
		else
#endif
...
CharacterManagerDlg::InitCharList()
{
	...
	cmdCharList.RemoveAllDBItems();				// clear prior selection if any (after delete)
	...
	AbleItem( Item( cmdCreateBtn ), cmdCharList.CountCharacters() < maxChars && not cmdIsDemo );
}
...
CharacterManagerDlg::SaveInfo() const
{
	...
	if ( noErr == result )
		...
		StringCopySafe( gPrefsData.pdCharName, joinname, sizeof gPrefsData.pdCharName );
...
CharacterManagerDlg::DoCreateLoop( const char * charpass )
{
	...
	if ( noErr == result )
		...
		size = dst - reinterpret_cast<char *>( &lo );
		result = SendWaitReplyLoop( challenge, &cmdMsg, size, GetSpinnerView() );
		}
	if ( noErr == result )
		result = BigToNativeEndian( lo.logResult );
...
CharacterManagerDlg::GetSelectedName( char * dst, size_t dstLen ) const
{
	...
	if ( noErr == result )
		...
		if ( CFStringRef name = cmdCharList.GetNthCharName( idx ) )
			{
			if ( not CFStringGetCString( name, dst, dstLen, kCFStringEncodingMacRoman ) )
				result = paramErr;
			}
		}
...
SelectCharacterDlg::FieldDidChangeText( HIViewRef vu )
{
	...
	if ( vu == nameVu )
		...
		if ( noErr == KeychainAccess::Get( kPassType_Character, name, pass, sizeof pass )
		&&   pass[ 0 ] )
			{
...
ChangePasswordDlg::Validate() const
{
	...
	if ( badField )
		{
		GenericError( _(TXTCL_MUST_FILL_FIELD) );
		__Verify_noErr( HIViewSetFocus( GetItem( Item( badField) ), kControlEditTextPart, 0 ) );
		return false;
		}
...
ProxyDlg::Init()
{
	// 'use proxy' is true if the user's proxy address matches the client's host address
	// and neither matches the default host address
	int value = 0;
	...
	// make port field visible only if we're NOT letting TCP/IP choose an arbitrary port
	...
	switch ( cmd.commandID )
		...
		case cmd_UsePort:
			if ( GetValue( Item(proxyUseSpecificPortCheck) ) )
				...
				// checkbox OFF: TCP/IP picks its own port, so hide the edit field
...
# endif
			...
			// turn off "Use alternate address" check if they chose "restore defaults"
			SetValue( Item(proxyUseProxyCheck), kHICommandRevert == cmd.commandID ? 0 : 1 );
			
			SetupProxy();
			
# ifndef DEBUG_VERSION
			// for non-GM client, "restore defaults" also disables "use specific port" feature
...
ProxyDlg::SaveInfo() const
{
	...
	GetText( Item( proxyAddressEdit ), addr, sizeof addr, kCFStringEncodingASCII );
	...
	StringCopySafe( gPrefsData.pdProxyAddr, addr, sizeof gPrefsData.pdHostAddr );
	
	// ... and also in the prefs' _host_ addr field, but only if the checkbox is on
	if ( bUseProxy )
		StringCopySafe( gPrefsData.pdHostAddr, addr, sizeof gPrefsData.pdHostAddr );
	else
		{
		// otherwise use the default server address
		StringCopySafe( gPrefsData.pdHostAddr, kDefaultHostAddrInternet,
			sizeof gPrefsData.pdHostAddr );
		}
...
**
**	as above but for CL character names: 3-15 [MacRoman] characters, all from VALID_NAME_CHARS
*/
bool
...
*/
DTSError
SendWaitReplyLoop( char * challenge, Message * msg, size_t size, HIViewRef spinVu /* = nullptr */ )
{
	DTSError result( noErr );
	Message msgsave;
	...
	for(;;)
		...
		msgsave = *msg;
		result = SendWaitReply( &msgsave, size, msgsave.msgLogOn.logData );
