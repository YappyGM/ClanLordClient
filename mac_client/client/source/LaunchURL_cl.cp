...
**	local definitions
*/
const Style kLaunchURL_LinkFace = 0x80;	// an unused value in the {normal,bold,italic...} range
...
LaunchURLHandler::Init()
{
	...
	for ( int i = 0;  i < kLaunchHandler_NumHelpers;  ++i )
		...
		// the actual URL values here are meaningless; we're only interested in whether
		// LaunchServices can handle a particular _scheme_.
		int len;
		if ( kLaunchHandler_MAILTOHelper != i )
			len = snprintf( buff, sizeof buff, "%s://www.deltatao.com/", helpers[i].scheme );
		else
			{
			// But we still need to special-case the email one, 
			// since <mailto://www.deltatao.com/> is malformed (no "@").
			len = snprintf( buff, sizeof buff, "%s:joe@deltatao.com", helpers[i].scheme );
			}
		
		if ( CFURLRef url = CFURLCreateWithBytes( kCFAllocatorDefault, (uchar *) buff, len,
							kCFStringEncodingASCII, nullptr ) )
			{
#if DEBUG_URL_INIT
			CFURLRef appURL = nullptr;
			OSStatus err = LSGetApplicationForURL( url, kLSRolesAll, nullptr, &appURL );
			if ( appURL )
				{
				CFShow( appURL );
				CFRelease( appURL );	// must release even tho' it came from a "Get" function:
										// see <LSInfo.h>
				}
#else
			OSStatus err = LSGetApplicationForURL( url, kLSRolesAll, nullptr, nullptr );
...
/*			"You do not have a helper application choosen for the type '%s'.\r\r"
	...
	if ( CFURLRef u = CFURLCreateWithBytes( kCFAllocatorDefault,
					(uchar *) url2, strlen(url2), kCFStringEncodingMacRoman, nullptr ) )
		{
...
FindProtocol( const char * myFindIn, const char * schemeToFind )
{
	...
	while ( mFound )
		{
		// If it is the first character, or the character before is in our "break" string
		if ( mFound - 1 < myFindIn
		||   strchr( kLaunchURLHandler_Breaks, *(mFound - 1) ) )
			{
			// If there isn't an alphanumeric character before the next break character
			const uchar * end = reinterpret_cast<const uchar *>( mFound ) + strlen( schemeToFind );
...
*/
OSStatus
LaunchURLHandler::ParseForURL( const char * data, long * urlStart, long * urlEnd ) const
{
...
#else
# define HELP_URL	CFSTR("http://www.deltatao.com/clanlord/index.html")
#endif
	...
	if ( 1 == LoWord( menuCmd ) )
		{
		if ( CFURLRef url = CFURLCreateWithString( kCFAllocatorDefault, HELP_URL, nullptr ) )
			{
...
**	IsLink()
**
**	Checks the unused bit of the face of the text - if it's set, then this is a link
*/
bool 
...
**
**	Underlines a URL and sets a disabled or enabled color over the current text face
...
URLStyledTextField::StyleTextURL( bool enabled )
{
	...
	//	Not having the last character count as a link prevents the style from continuing for
	//	the rest of the window if this is the last character
	
	//	Looking at this face bit of the character to the left of the one the mouse is on
	//	tells us if we should treat the mouse like it is on a link
