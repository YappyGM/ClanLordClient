...
**	Internal Routines
*/
static DTSError		Remove1Type( DTSKeyFile * file, DTSKeyType t, const DTSKeyType * goodTypes );
...
DataSpool::PutData( const void * buffer, size_t sz )
{
	...
	dsMark  = newmark;
	dsLimit = newmark;
	std::memcpy( dsData + oldmark, buffer, sz );
}
...
//	int32_t val = NativeToBigEndian( value );	// NOPE! if we do this, we'll be spooling
	...
	while ( bitstowrite > 0 )
		...
		uint8_t newbyte = uint8_t( uint32_t(val) >> (kBitsPerWord - CHAR_BIT) ); 	// high 8 bits
...
SafeString::ExpandTo( size_t newsize )
{
	...
	std::memcpy( newptr, ssPtr, ssSize );
...
SafeString::VFormat( const char * format, va_list params )
{
	...
	// (remember that vsnprintf() returns the number of characters it _would_ have written
	...
	if ( noErr == result )
		{
		/* len = */ vsnprintf( ssPtr + oldsize - 1, uint( len ) + 1, format, saved_params );
		ssSize = newsize;
		}
	va_end( saved_params );
}
...
CLImage::CalculateChecksum( DTSKeyFile * file, uint32_t * sum )
{
	...
	if ( noErr == result )
		result = file->Read( kTypePictureColors, cliPictDef.pdColorsID, colors, sizeof colors );
	
	// load optional lighting data
	if ( noErr == result
	&&	 cliPictDef.pdLightingID )
		{
		result = file->Read( kTypeLightingData, cliPictDef.pdLightingID, &light, sizeof light );
		if ( noErr == result )
			{
...
**	The image is expected to have a valid (and native-endian) cliPictDef; likewise
**	the bitsDef header data (6 bytes) should be native, as LoadImage() would have 
**	left it.  The color data never needs swapping.  The LightingData (which is optional)
**	must also already be native endian, if present.
*/
DTSError
...
//	||	 not ilInfo	// NULL light data just means "don't checksum it"
	...
	if ( noErr == result )
		...
		// which means that the otherwise identical image will nonetheless be given
		// an entirely new checksum value.
...
#else
		uint32_t tsum = NativeToBigEndian( static_cast<uint32_t>( (s2 << 16) + (s1 & 0x0FFFF) ) );
...
#else
	uint32_t packData =  ( static_cast<uint32_t>( pictID ) << (2 * kPictFrameCoordNumBits) )
...
{
#if BITWISE_IMAGE_SPOOL
	UnspoolFramePicture( spool, pictID, horz, vert, kPictFrameIDNumBits, kPictFrameCoordNumBits );
#else
...
{
	// GetUnusedID() will return an ID of 0 if there aren't any records of this type yet
	DTSKeyID test;
...
{
	static const DTSKeyType ciTypes[] = { kTypePictureBits, kTypePictureColors, kTypeClientItem,
		kTypeLightingData, kTypeLayout, kTypePictureDefinition, kTypeVersion, 0 };
...
**	returns length of correct language's text (not including null termination, if any)
**	if genderout is not null, sets it to linguistic gender constant
**	based on function written by Thomas Niederreiter, torx@arindal.com
*/
int
